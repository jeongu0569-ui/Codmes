...
def _client_cache_path() -> Path:
    return get_hermes_home() / "cache" / "google_antigravity_client.json"
...
def _credentials_to_auth_state(creds: GoogleCredentials) -> dict[str, Any]:
    return {
        "tokens": {
...
def _credentials_from_auth_state(state: dict[str, Any]) -> Optional[GoogleCredentials]:
    tokens = state.get("tokens") if isinstance(state.get("tokens"), dict) else state
    access = str(tokens.get("access_token") or tokens.get("accessToken") or "")
    refresh = str(tokens.get("refresh_token") or tokens.get("refreshToken") or "")
    if not access and not refresh:
        return None
    return GoogleCredentials(
        access_token=access,
        refresh_token=refresh,
        expires_ms=_coerce_int(
            state.get("expires_at_ms")
            or state.get("expires_ms")
            or tokens.get("expires_at_ms")
            or tokens.get("expires_ms")
            or 0
        ),
        email=str(state.get("email") or tokens.get("email") or ""),
        project_id=str(state.get("project_id") or tokens.get("project_id") or ""),
        managed_project_id=str(
            state.get("managed_project_id") or tokens.get("managed_project_id") or ""
        ),
    )


def _credentials_to_result(creds: GoogleCredentials) -> Dict[str, Any]:
    return {
        "access_token": creds.access_token,
        "refresh_token": creds.refresh_token,
        "expires_at_ms": creds.expires_ms,
...
def _load_credentials_from_auth_store() -> Optional[GoogleCredentials]:
    try:
        from hermes_cli import auth as hermes_auth

        auth_store = hermes_auth._load_auth_store()
        state = hermes_auth._load_provider_state(auth_store, PROVIDER_ID)
    except Exception:
        return None
    if not isinstance(state, dict):
        return None
    return _credentials_from_auth_state(state)


def _save_credentials_to_auth_store(creds: GoogleCredentials) -> bool:
    try:
        from hermes_cli import auth as hermes_auth

        with hermes_auth._auth_store_lock():
            auth_store = hermes_auth._load_auth_store()
            hermes_auth._save_provider_state(
                auth_store,
                PROVIDER_ID,
                _credentials_to_auth_state(creds),
            )
            hermes_auth._save_auth_store(auth_store)
        return True
    except Exception:
        return False


def _populate_identity_from_userinfo(creds: GoogleCredentials) -> GoogleCredentials:
    if not creds.access_token or creds.email:
        return creds
    try:
        import requests
    ...
    if email:
        creds.email = email
        _save_credentials_to_auth_store(creds)
    return creds


def describe_credentials(creds: GoogleCredentials) -> str:
    creds = _populate_identity_from_userinfo(creds)
    account = creds.email or "unknown Google account"
    details = [account]
    if creds.project_id:
        details.append(f"project {creds.project_id}")
    return " · ".join(details)


def _clear_auth_store_credentials() -> bool:
    try:
        from hermes_cli import auth as hermes_auth
...
def _update_auth_store_project_ids(project_id: str = "", managed_project_id: str = "") -> bool:
    try:
        from hermes_cli import auth as hermes_auth

        with hermes_auth._auth_store_lock():
            auth_store = hermes_auth._load_auth_store()
            state = hermes_auth._load_provider_state(auth_store, PROVIDER_ID) or {}
            if not isinstance(state, dict):
                state = {}
            if project_id:
            ...
            hermes_auth._save_provider_state(auth_store, PROVIDER_ID, state)
            hermes_auth._save_auth_store(auth_store)
        return True
    except Exception:
        return False
...
def load_credentials() -> GoogleCredentials | None:
    return _load_credentials_from_auth_store()


def save_credentials(creds: GoogleCredentials) -> None:
    if not _save_credentials_to_auth_store(creds):
        raise GoogleOAuthError(
            "Could not save Antigravity credentials to Hermes auth.json.",
            code="antigravity_auth_store_save_failed",
        )
    _mirror_credentials_to_cli(creds)


def clear_credentials() -> None:
    _clear_auth_store_credentials()
    ...
    try:
        if not token:
            ...
            with _antigravity_profile():
                original_save = google_oauth.save_credentials
                try:
                    google_oauth.save_credentials = lambda _creds: None
                    creds = google_oauth._refresh_access_token(creds)
                finally:
                    google_oauth.save_credentials = original_save
            save_credentials(creds)
            token = creds.access_token
    except GoogleOAuthError:
        # Standard OAuth refresh failed — likely client_secret mismatch with
        # the secret extracted from the agy binary. Fall back to agy CLI which
        # manages its own secrets internally.
...
def _prompt_existing_credentials_action(creds: GoogleCredentials) -> str:
    label = describe_credentials(creds)
    choices = [
        f"Use existing credentials ({label})",
    ...
    if sys.stdin.isatty():
        try:
            ...
            idx = curses_radiolist(
                ...
                selected=0,
                cancel_returns=2,
                description=f"Current Google account: {label}",
            )
            if not isinstance(idx, int) or idx < 0 or idx > 2:
                idx = 2
            return ("existing", "reauth", "cancel")[idx]
        except Exception:
            print()
            print("Google Antigravity credentials found:")
            print(f"  Current Google account: {label}")
            print()
    ...
    print(f"Using existing Google Antigravity credentials ({label}).")
    return "existing"


def choose_or_run_antigravity_oauth_login() -> Dict[str, Any]:
    existing = load_credentials()
    if existing is not None:
        try:
            get_valid_access_token()
            existing = load_credentials() or existing
            action = _prompt_existing_credentials_action(existing)
            if action == "existing":
                return _credentials_to_result(existing)
            if action == "cancel":
    ...
    return _credentials_to_result(creds)
...
def start_oauth_flow(
    *,
    ...
    with _antigravity_profile():
        ...
        if not creds.refresh_token and existing and existing.refresh_token:
...
def update_project_ids(project_id: str = "", managed_project_id: str = "") -> None:
    if _update_auth_store_project_ids(
        project_id=project_id,
