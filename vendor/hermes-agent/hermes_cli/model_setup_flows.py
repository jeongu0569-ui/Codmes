...
def _model_flow_ollama_local(config, current_model=""):
    """Configure a local Ollama server without treating it as a custom API."""
    import json
    ...
    configured = load_config().get("model") or {}
    current_base = ""
    if isinstance(configured, dict) and configured.get("provider") == "ollama-local":
        current_base = str(configured.get("base_url") or "").strip()

    raw_host = os.getenv("OLLAMA_HOST", "").strip() or current_base or "http://127.0.0.1:11434"
    if raw_host.rstrip("/").endswith("/v1"):
        raw_host = raw_host.rstrip("/")[:-3]
    raw_host = raw_host.rstrip("/")

    print()
    print("Ollama Local configuration:")
    try:
        override = input(f"Ollama server URL [{raw_host}]: ").strip()
    except (KeyboardInterrupt, EOFError):
        print()
        return
    host = (override or raw_host).rstrip("/")
    if host.endswith("/v1"):
        host = host[:-3]
    ...
    try:
        ...
        for item in payload.get("models", []):
            ...
            if capabilities and not any(cap in capabilities for cap in ("completion", "tools", "thinking")):
                continue
        ...
        print("Start Ollama and install a chat model, then try again.")
        return

    if not model_list:
        print("Ollama is connected, but no chat-capable local model was found.")
        return
    ...
    save_config(cfg)
    deactivate_provider()
    print(f"Default model set to: {selected} (via Ollama Local)")
