...
Codmes Search의 전체 흐름과 Ollama/LM Studio VLM 검증 기록은 [docs/search/codmes-search-explained.md](docs/search/codmes-search-explained.md)에 쉽게 정리되어 있습니다.
PDF 필기와 텍스트/이미지 annotation을 Windows/Android까지 가져가기 위한
공통 저장 계약은 [docs/notes/pdf-annotations.md](docs/notes/pdf-annotations.md)에 정리되어 있습니다.
...
<Workspace>/Notes/mypage.pdf
<Workspace>/Notes/.codmes/annotations/mypage.codmes.json  mypage.pdf의 편집 가능한 PDF 주석
```

파일에 직접 붙는 PDF 주석 상태는 문서 폴더 안의 숨김 `.codmes/annotations`
폴더에 저장합니다. 그래서 `Notes/`나 특정 강의 폴더를 통째로 옮기면 원본
PDF와 편집 가능한 필기/텍스트박스/이미지 오브젝트가 같이 따라가지만, 일반
파일 목록에는 PDF만 깔끔하게 보입니다. 전역 채팅 세션, 모델 인증, 승인,
장기 메모리처럼 앱 전체에 붙는 상태는 Workspace 루트 `.codmes`에 남습니다.
이전 버전의 `<Workspace>/.codmes/annotations` 또는 content-root
`.codmes/annotations`에 있던 주석 파일은 읽을 때 새 문서 폴더 기준 위치로
자동 복사됩니다.

파일 라이프사이클 동기화는 서버 API 테스트로 고정합니다. PDF/문서 파일을
이름 변경하거나 폴더 안에서 이동하면 연결된 annotation JSON도 새
workspace-relative path 기준으로 옮겨지고, 복사하면 주석도 새 문서용으로
복제되며, 삭제하면 연결된 주석 상태도 제거됩니다. 파일 생성, 수정,
업로드, chunked upload 완료, 주석 저장, 이동, 삭제는 Codmes Search 부분
인덱스를 즉시 갱신합니다. 여러 파일을 동시에 업로드하거나 수정할 때 검색
인덱스 파일이 서로 덮어쓰지 않도록 서버가 부분 인덱스 갱신을 직렬화합니다.

iOS/iPadOS PDF export/import는 페이지 단위 작업도 지원합니다. Export 화면의
`Pages` 입력란에 `1-3, 5`처럼 적으면 해당 페이지만 PDF로 내보내고, Codmes
state export를 선택하면 선택한 페이지만의 필기, 텍스트 박스, 이미지
오브젝트가 새 페이지 번호 기준으로 재매핑됩니다. PDF 뷰어의 삽입 버튼은
선택한 PDF와 선택적 `.codmes.json` 상태 파일을 현재 페이지 뒤에 삽입합니다.
일반 PDF를 삽입하면 주석 없이 페이지가 들어가고, 서버는 변경된 PDF를 다시
저장한 뒤 검색 인덱스를 갱신합니다. 현재는 삽입 후 문서 단위 재인덱싱이며,
삽입된 페이지만 OCR/임베딩하는 page-level cache는 다음 단계입니다.

필기 저장 포맷은 플랫폼 중립을 목표로 합니다. iOS/iPadOS와 macOS는
PDFKit ink annotation으로 저장된 stroke를 즉시 표시하고, 편집 가능한 원본
상태는 정규화된 `inkStrokes` JSON으로 기록합니다. `inkStrokes`는 0~1 좌표,
압력, 시간, 색상, 두께를 담는 Codmes 공통 stroke 포맷이므로 macOS, Windows,
Android/Galaxy Tab 클라이언트가 PencilKit 없이도 같은 필기 데이터를 렌더링할
수 있습니다. 현재 iOS/iPadOS 경로는 PencilKit overlay에 의존하지 않고
`PDFView`의 drawing gesture, live overlay, PDFKit `.ink` annotation, Codmes
annotation 저장 API를 사용합니다. 지우개는 stroke 전체 삭제가 아니라 eraser
radius 주변을 잘라 남은 stroke segment를 보존합니다. 읽기 모드에서는 PDFKit이
스크롤/확대를 맡고, 쓰기 모드에서는 iPad의 Pencil 입력과 iPhone의 한 손가락
입력을 도구 입력으로 사용합니다. iPad 쓰기 모드의 손가락 스크롤과 iPhone
쓰기 모드의 두 손가락 스크롤/줌 정책은 별도로 적용됩니다.
...
- PDF는 Apple PDFKit 기반으로 열고, iOS/iPadOS에서는 live drawing overlay와 PDFKit `.ink` annotation으로 펜 stroke를 즉시 표시한 뒤 문서 폴더의 `.codmes/annotations`에 `inkStrokes`를 저장합니다. 현재 PDF 필기 UX는 펜, 펜 색상/굵기 선택, 부분 지우개, 지우개 굵기 선택, 읽기/쓰기 모드, 텍스트 박스, 이미지 첨부, 텍스트/이미지 오브젝트 이동/리사이즈/삭제, 범위 export, Codmes state export, 현재 페이지 뒤 PDF/state 삽입을 지원합니다. 텍스트/이미지 오브젝트는 Codmes annotation bbox로 저장되어 검색 인덱스와 연결됩니다. 손글씨 stroke 자체의 OCR, Windows/Android용 공통 stroke 렌더러, 정교한 레이어 패널, 도형/스티커 도구, PDF 표준 annotation round-trip, 페이지별 크기가 다른 PDF의 export 품질 보강, 페이지 단위 OCR/임베딩 캐시는 남은 작업입니다.
...
- [Notes 문서](docs/notes/README.md)
- [Search/RAG 문서](docs/search/README.md)
- [RAG 백엔드 설계](docs/search/rag-backend-design.md)
- [Codmes Search 연동](docs/search/codmes-search-integration.md)
