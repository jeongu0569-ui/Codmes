...
6. Code 화면에서는 `Code Agent` 패널을 열어 현재 Code 폴더 기준 inspect task를
   만들 수 있다. 패널은 최근 code task, task memory, patch proposal diff,
   승인 적용, check 실행을 Workspace Server API로 처리한다.
...
Code 화면의 파일 브라우저에는 `Code Agent` 패널이 있다.
